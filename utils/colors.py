...
def generate_colors_from_uniform_distribution(size, low=0, high=1):
    while True:
        points = torch.zeros([size[0]*2, *size[1:]]).uniform_(low, high) 
        #points = points[torch.norm(points, dim=1) < 1]
        if points.shape[0] >= size[0]:
            return points[:size[0]]


def generate_colors(config, epoch, size, normalize_points=None):
    ...
    if normalize_points and config['target_network_input']['normalization']['type'] == 'progressive':
        ...
        points = generate_colors_from_uniform_distribution(size=size)
    ...
    def conect_points_with_edges(points):
        pass
