...
def sort_data_by_points(X):
    result = np.empty_like(X)
    for i, (obj, col) in enumerate(zip(X[:, :, :3], X[:, :, 3:6])):
        ind = np.lexsort((obj[:,0], obj[:,1], obj[:,2]))
        sorted = np.concatenate((obj[ind, :], col[ind, :]), axis=1)
        result[i] = sorted
    return result
...
def _pairwise_EMD_MSE_(sample_pcs, ref_pcs, batch_size):
    N_sample = sample_pcs.shape[0]
    N_ref = ref_pcs.shape[0]
    all_cd = []
    iterator = range(N_sample)
    for sample_b_start in iterator:
        ...
        for ref_b_start in range(0, N_ref, batch_size):
            ...
            dist = mse_loss(sample_batch_exp, ref_batch)
            cd_lst.append(dist.view(1, -1))
...
def compute_all_metrics_colors(sample_pcs, ref_pcs, batch_size):
    results = {}

    M_rs_cd, M_rs_emd = _pairwise_EMD_MSE_(ref_pcs, sample_pcs, batch_size)

    res_cd = mmd_cov(M_rs_cd.t())
    results.update({
        "%s-MSE" % k: v for k, v in res_cd.items()
    })

    M_rr_cd, M_rr_emd = _pairwise_EMD_MSE_(ref_pcs, ref_pcs, batch_size)
    M_ss_cd, M_ss_emd = _pairwise_EMD_MSE_(sample_pcs, sample_pcs, batch_size)

    # 1-NN results
    one_nn_cd_res = knn(M_rr_cd, M_rs_cd, M_ss_cd, 1, sqrt=False)
    results.update({
        "1-NN-MSE-%s" % k: v for k, v in one_nn_cd_res.items() if 'acc' in k
    })
...
def unit_cube_grid_color_cloud(resolution, clip_sphere=False):
    """Returns the center coordinates of each cell of a 3D grid with resolution^3 cells,
    that is placed in the unit-cube.
    If clip_sphere it True it drops the "corner" cells that lie outside the unit-sphere.
    """
    ...
    for i in range(resolution):
        for j in range(resolution):
            for k in range(resolution):
                grid[i, j, k, 0] = i * spacing 
                grid[i, j, k, 1] = j * spacing 
                grid[i, j, k, 2] = k * spacing 
...
def jsd_between_point_cloud_sets(sample_pcs, ref_pcs, resolution=28, color=False):
    ...
    """
    in_unit_sphere = True
    sample_grid_var = entropy_of_occupancy_grid(sample_pcs, resolution, in_unit_sphere, color)[1]
    ref_grid_var = entropy_of_occupancy_grid(ref_pcs, resolution, in_unit_sphere, color)[1]
    return jensen_shannon_divergence(sample_grid_var, ref_grid_var)


def entropy_of_occupancy_grid(pclouds, grid_resolution, in_sphere=False, verbose=False, color=False):
    ...
    if color:
        grid_coordinates, _ = unit_cube_grid_color_cloud(grid_resolution, in_sphere)
    else:
